...
#define ANTTWEAKBAR_IMPLEMENTATION
...
inline GLuint LoadShaders(const char * vertex_file_path,
                          ...
                          const char * tc_file_path = NULL,
                          const char * te_file_path = NULL,
                          const char * geometry_file_path = NULL) {
    const int SHADER_LOAD_FAILED = 0;



    string vertex_shader_code, fragment_shader_code, tc_shader_code, te_shader_code,  geometry_shader_code;
    {
        ...
        if(tc_file_path != NULL) {
            ifstream tc_shader_stream(tc_file_path, ios::in);
            if(tc_shader_stream.is_open()) {
                tc_shader_code = string(istreambuf_iterator<char>(tc_shader_stream),
                                        istreambuf_iterator<char>());
                tc_shader_stream.close();
            } else {
                printf("Could not open file: %s\n", tc_file_path);
                return SHADER_LOAD_FAILED;
            }
        ...
        if(te_file_path != NULL) {
            ifstream te_shader_stream(te_file_path, ios::in);
            if(te_shader_stream.is_open()) {
                te_shader_code = string(istreambuf_iterator<char>(te_shader_stream),
                                        istreambuf_iterator<char>());
                te_shader_stream.close();
            } else {
                printf("Could not open file: %s\n", te_file_path);
                return SHADER_LOAD_FAILED;
            }
    ...
    char const *tc_source_pointer = NULL;
    char const *te_source_pointer = NULL;
    char const *geometry_source_pointer = NULL;
    if(tc_file_path != NULL) tc_source_pointer = tc_shader_code.c_str();
    if(te_file_path != NULL) te_source_pointer = te_shader_code.c_str();
    if(geometry_file_path != NULL) geometry_source_pointer = geometry_shader_code.c_str();

    int status = CompileShaders(vertex_source_pointer, fragment_source_pointer, tc_source_pointer,
                                te_source_pointer, geometry_source_pointer);
